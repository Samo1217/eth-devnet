...
print(f"[loadgen] boot: RPC_URL={RPC_URL} TPS={TPS} CONCURRENCY={CONCURRENCY} "
      f"TO={TO_ADDR} VALUE_ETH={VALUE_ETH} SAMPLE_WINDOW={SAMPLE_WINDOW} "
      f"RPS_BLOCK={RPS_BLOCK} RPS_BAL={RPS_BAL} RPS_CALL={RPS_CALL}", flush=True)
...
RPC_LAT = Histogram("loadgen_rpc_latency_seconds","RPC latency seconds",["method"],
...
async def get_sender(rpc: Rpc) -> str:
    ...
    if not accs:
        raise RuntimeError("No unlocked accounts returned by eth_accounts; geth --dev should expose one.")
    return accs[0]
...
async def run():
    ...
    try:
        ...
        for m in ["eth_sendTransaction", "eth_maxPriorityFeePerGas",
        ...
        if RPS_BAL > 0:
            asyncio.create_task(rps_loop(rpc, "eth_getBalance", lambda: [sender, "latest"], RPS_BAL))
        if RPS_CALL > 0:
            asyncio.create_task(rps_loop(rpc, "eth_call", lambda: [{"to": TO_ADDR}, "latest"], RPS_CALL))
